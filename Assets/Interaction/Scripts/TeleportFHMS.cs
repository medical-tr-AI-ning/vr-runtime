...
{
	//-------------------------------------------------------------------------
	public class TeleportFHMS : Teleport
    {

		public bool teleportFadeBlack = true;
		public bool teleportZoom = false;
		...
		public static new Teleport instance
		{
			get
			{
				if ( _instance == null )
...
/*#else
		...
		//-------------------------------------------------
		public new void HideTeleportPointer()
		{
			if ( pointerHand != null )
			{
				HidePointer();
		...
		private void TeleportPlayer()
		{
			...
			if ( teleportingToMarker.ShouldMovePlayer() )
			{
				...
				if(!teleportZoom) {
					...
					zoomCoroutine = teleportZoomCoroutine(player.trackingOriginTransform.position,teleportPosition + playerFeetOffset);
		...
		private IEnumerator teleportZoomCoroutine(Vector3 from, Vector3 to) {
			float t = 0.0f;

			while(t < 1f){
				player.trackingOriginTransform.position = Vector3.Lerp(from, to, t);
				
				yield return 0;
				
				t = t + Time.deltaTime/zoomTime;
			}

			player.trackingOriginTransform.position = to;
		}
		...
		//-------------------------------------------------
		public new void ShowTeleportHint()
		{
			CancelTeleportHint();
		...
		//-------------------------------------------------
		public new void CancelTeleportHint()
		{
		...
		//-------------------------------------------------
		public new bool IsEligibleForTeleport( Hand hand )
		{
			if ( hand == null )
			{
				return false;
		...
		public void setZooming(){
			teleportZoom = true;
			teleportFadeBlack = false;
		}

		public void setBlink(){
			teleportZoom = false;
			teleportFadeBlack = true;
		}
